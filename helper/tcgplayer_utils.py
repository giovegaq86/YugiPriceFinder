...
class TCGPlayerUtils:
    ...
    def get_items(self, set_code, card_name, edition="", condition="", image="", expansion="", rarity="",
                  product_id=""):
        try:
            ...
            for item in items:
                ...
                price = float(item.find_all('span', class_='product-listing__price')[0].text
                              .replace('$', '').replace(',', ''))
                pc = f'₡{format(float(price) * self.exchange_rate, ",.0f")}'
            ...
            print(f'Occurred the following error trying to get items: {e}')
            return []
    ...
    def get_card_info_from_card_name(self, card_name, set_code):
        ...
        for card in cards:
            if card['customAttributes']['number'] == set_code:
                ...
                print(f'get_card_info_from_card_name, product_id={product_id}, rarity={rarity}, expansion={expansion},'
                      f'image={image}')
                break
    ...
    def get_prices(self, set_code, edition="", condition=""):
        try:
            ...
            if card_info:
                ...
                product_id, rarity, expansion, image = self.get_card_info_from_card_name(card_name=card_name, set_code=set_code)
            ...
            print(f'Occurred the following error trying to get prices: {e}')
            raise Exception(e)
