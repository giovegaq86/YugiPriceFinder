...
class TYTUtils:
    ...
    def get_card_info(self, card_key, edition, condition, hide_oos=False):
    ...
    def get_prices(self, set_code, edition, condition, language, hide_oos=False):
        print(f'T&T - Getting prices from set code: {set_code}')
        # Get card info
        cards = self.get_card_info(card_key=set_code, edition=edition, condition=condition, hide_oos=hide_oos)
        card_list = []
        ...
        if len(cards) > 0:
            for card in cards:
                ...
                edition_ = utils.get_edition(card_info)
                rarity = card_info.split(' - ')[-1].replace(edition_, '').strip()
                card_name = card_info.replace(set_code, '').replace(rarity, '') \
                    .replace('-', '').replace(edition_, '').strip()
                ...
                for item in items:
                    ...
                    pc = f'₡{format(float(p) * self.exchange_rate, ",.0f")}'
